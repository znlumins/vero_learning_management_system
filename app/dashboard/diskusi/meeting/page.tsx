...
import { 
  ...
  ChevronLeft, Hand, MessageSquareText, Volume2, VolumeX, 
  Layout, Pencil, Eraser, Trash2, Undo2, Redo2, MonitorPlay, Upload, ChevronRight, Move
} from "lucide-react";
...
function MeetingContent() {
        ...
        if (now - lastPredictionTime.current > 600) {
            ...
            try {
                const res = await fetch("http://127.0.0.1:5000/api/predict", {
                    ...
                    body: JSON.stringify({ landmarks_list: results.multiHandLandmarks, handedness_list: results.multiHandedness, model_type: modelTypeRef.current }),
                });
                const d = await res.json();
                if (d.label && d.label !== "--") updateMySubtitle(d.label.toUpperCase());
            } catch (err) {}
        }
    }
    ctx.restore();
  ...
  const toggleGesture = async () => {
    if (isGestureActive) { gestureEnabledRef.current = false; setIsGestureActive(false); }
        ...
        if (!window.myHandsInstance) {
            ...
            window.myHandsInstance.setOptions({ maxNumHands: 2, modelComplexity: 1, minDetectionConfidence: 0.5 });
  ...
  const leaveMeeting = () => { window.location.href = "/dashboard/diskusi"; };
  ...
  return (
    <div className="flex flex-col h-screen bg-slate-950 font-sans text-white overflow-hidden">
      ...
      <header className="h-14 px-6 bg-slate-900 border-b border-white/10 flex items-center justify-between z-50">
        ...
        <div className="flex items-center gap-3">
            <div className="flex bg-slate-800 p-1 rounded-xl border border-white/5">
                {['bisindo', 'sibi'].map((m) => (
                    <button key={m} onClick={() => setModelType(m as any)} 
                    className={`px-3 py-1 rounded-lg text-[10px] font-bold uppercase transition-all ${modelType === m ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-500'}`}>
                    {m}
                    </button>
                ))}
            </div>
            ...
            <button onClick={togglePresentation} className={`flex items-center gap-2 px-4 py-1.5 rounded-xl text-[10px] font-bold uppercase transition-all border ${isPresenting ? 'bg-green-500 border-green-500 text-white' : 'border-white/10 text-white hover:bg-white/5'}`}>
                <MonitorPlay size={16} /> {isPresenting ? "Stop Presentasi" : "Presentasi"}
            </button>
      ...
      <div className="flex-1 flex overflow-hidden p-4 gap-4 relative">
        <div className={`flex-1 transition-all duration-700 flex flex-col ${activePresImage || isWhiteboardOpen ? 'bg-white rounded-[40px] shadow-2xl overflow-hidden text-slate-900' : ''}`}>
           {activePresImage ? (
               <div className="relative flex-1 flex flex-col bg-slate-50">
                  <div className="flex-1 relative p-8">
                     ...
                     {(mySubtitle || peers[remotePresentation?.peerId]?.subtitle) && (
                        <div className="absolute bottom-10 left-1/2 -translate-x-1/2 bg-slate-900 text-white px-8 py-3 rounded-3xl text-2xl font-black italic shadow-2xl border-4 border-indigo-500">
                           {mySubtitle || peers[remotePresentation?.peerId]?.subtitle}
                        </div>
                     )}
                  </div>
               ...
               <div className="flex-1 flex flex-col">
                  ...
                  <ReactSketchCanvas ref={canvasRef} strokeColor="#4f46e5" strokeWidth={4} onStroke={() => {
                     if(!isIncomingDrawing.current) {
                        canvasRef.current?.exportPaths().then(p => broadcastData({ type: 'whiteboard', action: 'draw', paths: p }));
                     }
                  }} />
               </div>
           ) : (
               <div className="flex-1 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  <div className="relative aspect-video bg-slate-900 rounded-3xl overflow-hidden border border-white/5">
                     ...
                     {mySubtitle && <div className="absolute bottom-6 left-1/2 -translate-x-1/2 bg-black/80 px-4 py-2 rounded-xl text-lg font-bold">{mySubtitle}</div>}
                  </div>
        ...
        {(activePresImage || isWhiteboardOpen) && (
            <div className="w-64 flex flex-col gap-4 overflow-y-auto pr-2">
                <div className="relative aspect-video bg-slate-800 rounded-2xl overflow-hidden border-2 border-indigo-500 shadow-lg">
                    {myStream && <VideoPlayer stream={myStream} isMuted={true} />}
                    {isGestureActive && <canvas ref={drawCanvasRef} className="absolute inset-0 w-full h-full object-cover scale-x-[-1]" width={640} height={480} />}
                </div>
                {Object.entries(peers).map(([id, p]) => (
                    <div key={id} className="relative aspect-video bg-slate-800 rounded-2xl overflow-hidden border border-white/5">
                        <VideoPlayer stream={p.stream} />
                    </div>
                ))}
            </div>
      ...
      <div className="h-20 bg-slate-950 border-t border-white/5 flex items-center justify-center gap-4 md:gap-8 shrink-0 z-50">
        ...
        <button onClick={leaveMeeting} className="px-10 py-4 bg-red-600 hover:bg-red-700 text-white rounded-3xl font-black text-xs uppercase tracking-widest shadow-xl active:scale-95 transition-all">Keluar</button>
      </div>
    </div>
  );
}
